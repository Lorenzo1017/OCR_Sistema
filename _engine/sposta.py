...
from ocrsys.naming import resolve_collision


def _trova(db, frammento: str) -> list:
    cur = db.conn.execute(
        "SELECT sha256, nome_file, percorso, categoria FROM documenti "
        "WHERE nome_file LIKE ? ORDER BY nome_file", (f"%{frammento}%",))
    return cur.fetchall()
...
def main():
    ...
    try:
        ...
        if len(trovati) > 1:
            ...
            for _, nome, _, cat in trovati[:20]:
                print(f"  [{cat}] {nome}")
            return
        sha, nome, percorso, cat_vecchia = trovati[0]
        ...
        if cat_vecchia == categoria:
            ...
            return
        dest_dir = config.ARCHIVIO / categoria
